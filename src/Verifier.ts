...
export const verify = async (pdfArrayBuffer: ArrayBuffer, nodeUrl: string = '') => {
...
const availableContracts: string[] = ['0x5d305D8423c0f07bEaf15ba6a5264e0c88fC41B4',
...
export const extractMetadata = async (pdfInfo: { Custom: { verifymn: string } }) => {
...
async function _validateInner(metadata: MetaDataInterface, pdfString: string, nodeUrl: string) {
  ...
  try {
    ...
    if (!availableContracts.find((x: string) => x.toLowerCase() === smartContractAddress.toLowerCase())) {
      isValid = false;
    } else {
      if (version.includes('university')) {
        return _validateUniversity(PDFHash, metadata, isTestnet, nodeUrl);
      }
      const certification = await requestCertificationByHash(PDFHash, smartContractAddress, isTestnet, nodeUrl);
      if (certification.hash !== PDFHash) {
        ...
        try {
          result.issuer = await requestIssuerByAddress(certification.issuer, isTestnet, nodeUrl);
...
async function _validateUniversity(PDFHash: string, metadata: MetaDataInterface, isTestnet: boolean, nodeUrl: string) {
  const univMeta = jsonWrap(metadata['univ_meta']);
  let result: VerifyResultInterface = { ...defaultResult, isTestnet: isTestnet, isUniversity: true, metadata: metadata };
  const utf8 = require('utf8');
  const x = utf8.encode(univMeta);
  const metaHash = await extractHash(x);
  const certInfo = await requestUniversityCertByHash(PDFHash, metadata.blockchain.smartContractAddress, isTestnet, nodeUrl);
  ...
  try {
    result.issuer = await requestIssuerByAddress(certInfo.cert.issuer, isTestnet, nodeUrl);
  } catch (e) {
    console.error(e);
